...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Automate video chunking and AI-driven analysis.",
    )
    parser.add_argument(
        "video",
        nargs="?",
    ...
    parser.add_argument(
        ...
        help="Model identifier for the video analysis step.",
    )
    parser.add_argument(
        "--planner-model",
        default="Qwen3-30B-A3B",
        help="Model identifier for the director planning step.",
    )
    parser.add_argument(
        "--api-key",
        default=None,
        help="DashScope/Qwen API key. Falls back to DASHSCOPE_API_KEY env variable.",
    )
    parser.add_argument(
        "--final-output",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Do not call remote models or execute ffmpeg. "
            "Generates placeholder data for testing the control flow."
        ),
    )
    parser.add_argument(
        "--use-file-picker",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    assembler = VideoAssembler(ffmpeg=config.ffmpeg, dry_run=args.dry_run)
